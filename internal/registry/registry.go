...
// cannot be reached. The VS Code extension (entrypoint "claude-vscode") is
// the same shape: the editor drives the process over stdio and shows it in
// a webview no terminal owns. Every other entrypoint is assumed focusable,
// recorded or not: an unknown shape should degrade to showing more, not less.
func (a Agent) Focusable() bool {
	return !strings.HasPrefix(a.Entrypoint, "sdk-") && a.Entrypoint != "claude-vscode"
}
